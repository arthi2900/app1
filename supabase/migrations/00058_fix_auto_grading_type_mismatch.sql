-- Fix Auto-Grading Type Mismatch
-- Date: 2025-01-21
-- Issue: student_answer is jsonb, correct_answer is text
...
BEGIN
  ...
  LOOP
    ...
    IF question_record.question_type IN ('mcq', 'true_false', 'multiple_response', 'match_following') THEN
      -- Compare answers: student_answer (jsonb) with correct_answer (text cast to jsonb)
      -- Use text comparison to handle Unicode characters properly
      is_correct := (answer_record.student_answer::text = question_record.correct_answer::jsonb::text);
