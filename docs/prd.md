...
5. **Insert Images/Clip Arts** (Image upload field, optional)
   ...
   - **File Selection**: Click button to open file browser
...
9. **Options** (Dynamic fields, conditional)\n   - For Multiple Choice: Four option input fields (Option1, Option 2, Option 3, Option 4) displayed simultaneously
   ...
   - For Short Answer and Essay: Options field is hidden\n10. **Correct Answer** (Text input, dropdown, or multi-select, required)
...
\n#### 6.3.3 Form Actions
- **Add Button**: Saves the question to question_bank table (including uploaded images)
...
- Typography:\n  - Question text: 16px, bold\n  - Metadata (class, subject, lesson): 14px, regular
...
- Form fields same as creation form (including image upload section)
- **Image Editing**:
...
- **Question Bank**: Access to create questions for assigned subjects and lessons with dual view options and image upload capability
...
- Question creation form follows same field order as Principal (Class → Subject → Lesson → Question → Insert Images/Clip Arts → Question Type → Marks → Difficulty → Options → Correct Answer)
- **Image Upload**: Teachers can insert images/clip arts in questions
- **Dual View Options**:
  - Row View (table format)
  - Card View (detailed card format)
- Teacher can view and edit questions they created in both views
- Questions are automatically filtered by teacher's assigned subjects\n- Edit functionality available in both Row View and Card View (including image editing)
...
- **Question Bank Management**:
  ...
  - Switch between Row View and Card View
  - Edit questions in both views (including image editing)
...
- **Question Bank Interface**: \n  - **View Toggle Control**: Toggle button or tab control at top of page to switch between Row View and Card View\n  - **Row View**: Compact table layout with sortable columns and Edit/Delete actions, with color-coded badges for all question types (including Match the Following - Teal, Multiple Response MCQ - Indigo), plus image indicator icon for questions with images
  - **Card View**: Detailed card layout displaying all question information with prominent Edit and Delete buttons
  ...
  - Question creation form with cascading dropdowns (Class → Subject → Lesson)\n  - **Image Upload Section**: 'Add Image' button with file browser, thumbnail preview, caption input, and delete option for each image
  ...
  - Question list view with lesson-level filtering in both views
